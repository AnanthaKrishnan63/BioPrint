...
def load_track(track: str, split: str, window: int = 50):
    ...
    if track == 'fixed':
        ...
        for subject, rows in grouped.items():
            ...
            out[subject] = np.asarray(features)
    for a in out.values():
        a[~np.isfinite(a)] = np.nan
    return out, names
...
def run(track, output: Path, window=50):
    ...
    dev, dev_names = load_track(track, 'dev', window)
    ...
    result = {'dataset': 'KeyRecs 2023', 'track': track, 'evaluation_split': 'dev',
              'development_n': len(dx), 'subjects': len(subjects), 'validation': validation,
