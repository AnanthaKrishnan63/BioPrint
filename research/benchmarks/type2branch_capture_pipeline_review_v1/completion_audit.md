...
| Each dataset under 500 MB; total under 5 GB | `dataset_budget_type2branch_capture.json`: 1,208,053,730 bytes; conservative largest-source bound 465,998,793 bytes; source documents included | Verified at recorded snapshot within declared scope |
...
| FAR/FRR/EER and improvement | `summary_v9.json`: 63 rows, 23 source reports; three infeasible experiments including fixed-capture Type2Branch | Verified measurements, not universal improvement or near-zero errors |
| Type2Branch short-capture follow-up | Frozen all79account protocol, raw-prefix TRAIN calibration features, generated-tested evaluator and API pipeline; equal-budget paired training still running | Actual calibration, DEV performance and trained-worker replay pending; original failed protocol preserved |
...
`type2branch_feasibility.md` documents missing extended-feature generation and
reference-runtime incompatibility; `pointer_lt_mouse_audit.md` documents missing
architecture and training specifications. These are concrete reproduction gaps,
not evidence that existing models are SOTA. `beacon_nonlinear/README.md` records
