...
DATASETS = ('cmu-account-selection', 'cmu-far-selection')
...
async def main(dataset='cmu-account-selection'):
    if dataset not in DATASETS:
        raise ValueError('Unsupported frozen dataset')
    ...
    with np.load(paths[2], allow_pickle=False) as archive:
        ...
        if dataset == 'cmu-far-selection':
            aliases['eer_reference'] = 'account_selected'
    ...
    result = {'dataset': DATASET, 'dev_samples': len(labels), 'enrolled_claims_per_sample': len(subjects),
              ...
              'replay_source_sha256': digest(Path(__file__)),
    ...
    with (DIRECTORY / 'api_replay_source.py').open('x') as stream:
