"""Prepare/review then run Selection-only tap extraction on exact frozen window IDs."""
import argparse
...
PROPOSAL=ROOT/'research/benchmarks/hmog/tap_selection_protocol_v1.json'
PROPOSAL_SHA='1e7872cd57b20642af24b7f1ad3af5ba90b4c84cdd9418f129bdbf68f957a116'
SUPPLEMENT=ROOT/'research/benchmarks/hmog_tap_implementation_contract_v1.md'
FIT=ROOT/'datasets/hmog/selection_features_v1.npz'
FIT_SHA='0f52812d33aa71bc4b158aa0c535b57d7e946af890befad7b2ac6a75b2f8882a'
PLAN=OUT/'selection_tap_extraction_plan_v1.json'
DEST=ROOT/'datasets/hmog/selection_tap_features_v1.npz'
IDS={'180679','622852'}
...
def validate_metadata(meta,expected_rows=60):
    ...
    if not set(meta['subject'].astype(str)).issubset(IDS):raise PermissionError('Only fixed selection identities allowed')
    for s,r,w in zip(meta['session'],meta['role'].astype(str),meta['window_index']):
        if not 1<=s<=16 or r!=('train_enrollment' if s<=8 else 'train_selection') or w<0:
...
def prepare():
    ...
    for relative,sha in proposal['source_hashes'].items():
        if digest(ROOT/relative)!=sha:raise ValueError('Proposal dependency changed: '+relative)
    meta=load_metadata()
    sources={n:digest(ROOT/'scripts'/n) for n in ['hmog_selection_tap_extract.py','hmog_tap_windows.py','hmog_tap_reference.py','hmog_contact_parser.py','hmog_data.py']}
    plan={'created_at_utc':datetime.now(timezone.utc).isoformat(),'proposal_sha256':PROPOSAL_SHA,
        ...
        'selection_window_archive_sha256':FIT_SHA,'sources_sha256':sources,'selection_window_count':60,
        'subjects':sorted(IDS),'phase':'tap_selection_feature_extraction','purpose':'tap_selection_feature_extraction',
        'raw_members':['Activity.csv','TouchEvent_im.csv'],'metadata_only_archive_fields':list(META),
        'window_rule':'Exact original activity wallstart+30000*window_index; all60originalrows retained',
        ...
        'min_scan_taps':5,'conventions':json.loads((ROOT/'research/benchmarks/hmog_tap_protocol_proposal_v1.json').read_text())['conventions'],'source_snapshot':'selection_tap_extraction_source_v1.py',
    ...
    print(json.dumps({'prepared_plan':str(PLAN.relative_to(ROOT)),'raw_members_read':False,'windows':60}))


def guarded_rows(reader,sid,session,filename):
    with reader.open_member(sid,session,filename,expected_cohort='selection',
        expected_role='train_enrollment' if session<=8 else 'train_selection',purpose='tap_selection_feature_extraction') as m:
...
def run():
    ...
    if digest(PROPOSAL)!=plan['proposal_sha256'] or digest(FIT)!=plan['selection_window_archive_sha256']:
    ...
    config=Conventions(**plan['conventions']);vectors=[];mapping=[];counts=np.zeros(60,dtype=np.int32)
    diagnostics={};groups={}
    ...
    report={'plan':plan,'original_window_count':60,'scorable_windows':int(np.sum(counts>=5)),
    ...
    with (OUT/'selection_tap_extraction_results_v1.json').open('x') as f:json.dump(report,f,indent=2)
    print(json.dumps({'original_windows':60,'scorable':report['scorable_windows'],'vectors':len(vectors)}))
