"""Hash-pinned local capture release; no model or recording loading."""
import hashlib
import json
...
RELEASE=ROOT/'research/benchmarks/type2branch_capture_release_v1'
FILES=frozenset(['encoder.index','encoder.data-00000-of-00001','profiles.npz','dev_features.npz','dev_report.json'])
SOURCES=frozenset(['scripts/type2branch_capture_worker.py','scripts/type2branch_capture_wire.py',
    'scripts/type2branch_wire.py','scripts/type2branch_capture_release.py',
    'scripts/type2branch_scoring.py','scripts/type2branch_reference_smoke.py',
...
def verify_release(expected_sha,directory=RELEASE):
    ...
    if manifest.get('version')!=1 or manifest.get('protocol')!='capture' or manifest.get('split')!='dev':
        raise ValueError('Wrong capture release protocol/version/split')
    ...
    for name,expected in manifest['sources'].items():
        if digest((ROOT/name).read_bytes())!=expected:raise ValueError('Release source changed')
    return manifest
