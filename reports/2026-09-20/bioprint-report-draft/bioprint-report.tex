...
\hypersetup{pdftitle={BioPrint: Adaptive Behavioral Verification},pdfauthor={Ananthakrishnan Thulasiraman},pdfsubject={Final six-page research and implementation report}}
...
{\normalsize\textbf{Ananthakrishnan Thulasiraman}\par}
\vspace{4pt}
...
Every entry in the next table is \textbf{FAR / FRR / EER (\%)}. The final column is baseline EER minus our method's EER, in percentage points; positive values indicate improvement. Table 1 explains the representation, baseline and our method behind each row.
\begin{center}\smalltable
\begin{tabularx}{\linewidth}{@{}Xrrr@{}}
\toprule
\textbf{Dataset / task} & \textbf{Baseline} & \textbf{Our method} & \textbf{EER gain (pp)}\\\midrule
...
\subsection{From statistical enrollment to learned typing recognition}
BioPrint starts with a personal statistical profile when enrollment data are limited: median timings and their variability provide an immediate reference without fitting a complex owner/background boundary. As sufficient compatible enrollment, background and calibration data become available, the RBF SVM provides a richer model of the owner's timing pattern. The CMU results support this progression: SVM EER falls from \textbf{14.32\% with ten enrollments to 9.14\% with 100}, while FRR falls from \textbf{67.43\% to 44.43\%} at approximately 1\% FAR. At the larger budget, SVM also outperforms the statistical baseline's 13.99\% EER. These measured configurations motivate data-dependent model selection; they do not prescribe a universal sample-count threshold for every password.

The working application demonstrates this progression through its compatible-model path. Temporary accounts enrolled with the public text \texttt{.tie5Roanl} are fitted and evaluated through the actual login API. A true accept/reject correctly recognizes an owner/impostor; a false reject/accept is the corresponding error. Table 3 illustrates these outcomes and the policy's response: model acceptance uses 1.0, while direct admission requires 0.75. The examples are selected demonstrations, not an accuracy estimate.
\begin{center}\small
...
\textbf{Trained pointer is the preferred direction for security.} Its \textbf{10.00\% synthetic FAR is the lowest} among the evaluated combinations; its 31.67\% FRR is below typing repetition's 41.67\%. Against statistical pointer, it prevents five false accepts at the cost of six net genuine failures. We prioritize this tradeoff because a false accept grants an impostor access, whereas a false reject interrupts a legitimate attempt. Trained pointer is therefore our chosen foundation, with further rejection reduction a usability priority.

\textbf{Unequal error costs explain the choice.} Let $c_{FA}$ and $c_{FR}$ be the costs of one false accept and one false reject. Trained pointer has lower cost on the paired counts when $5c_{FA}>6c_{FR}$, or $c_{FA}/c_{FR}>1.2$. This favors the trained model under our security priority. Both methods challenge 63/150 cases; the encoder decides 33 desktop cases. All 33 scores match frozen predictions within 0.000002, and all 300 paired session outcomes were checked. These findings support the selection without establishing population-wide statistical superiority from a small synthetic cohort.
