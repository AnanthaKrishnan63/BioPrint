...
def metric_difference(actual,expected):
    """Compare every nested metric, preserving missingness and categorical data."""
    if isinstance(expected,dict):
        if not isinstance(actual,dict) or set(actual)!=set(expected):
            raise ValueError('Metric fields differ')
        return max((metric_difference(actual[k],v) for k,v in expected.items()),default=0.)
    if isinstance(expected,list):
        if not isinstance(actual,list) or len(actual)!=len(expected):
            raise ValueError('Metric lists differ')
        return max((metric_difference(a,b) for a,b in zip(actual,expected)),default=0.)
    if type(expected) in (int,float):
        if type(actual) not in (int,float) or not np.isfinite(actual) or not np.isfinite(expected):
            raise ValueError('Metric numeric types or finiteness differ')
        return abs(actual-expected)
    if actual!=expected:raise ValueError('Metric categories or missingness differ')
    return 0.
...
async def replay():
    ...
    metric_error=metric_difference(computed,report['metrics'])
    ...
    result={'status':'actual_capture_dev_api_replay_complete','probes':total,'claims':scores.size,
        'max_absolute_score_error':error,'max_absolute_metric_error':metric_error,
        'decision_flips':flips,'gate_checks':checks,'metrics':computed,
