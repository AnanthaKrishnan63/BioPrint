"""One frozen CMU FAR-objective-selection dev comparison; no fitting or tuning."""
import hashlib
import json
...
def validate():
    ...
    assert digest(Path(cmu.__file__)) == artifact['protocol']['strict_source_sha256']
    original = ROOT / 'research/benchmarks/cmu-account-selection/frozen_models.json'
    assert digest(original) == artifact['protocol']['control_artifact_sha256']
    plan = {'artifact_sha256': digest(artifact_path), 'validator_sha256': digest(__file__),
            ...
            'caveat': 'Legacy and prior global/EER-account dev exposure; exploratory follow-up. Account bootstrap does not fully model dependence from shared impostor probes.'}
    ...
    result = {'protocol': artifact['protocol'], 'artifact_sha256': digest(artifact_path),
              'selected_on_training': 'far_selected', 'split': 'validation_dev', 'results': {},
    ...
    chosen = result['results']['far_selected']['per_user']
    for reference in ['baseline', artifact['protocol']['original_selected'], 'account_selected']:
        ...
        result['paired_account_deltas'][f'far_selected_minus_{reference}'] = {
