...
event prefixes, and shorter captures request additional verification. Two
equal-budget TRAIN-only training arms are in progress; the control arm is
complete and the mixed-length arm is still running. Raw-prefix calibration
features, completion gates, DEV preparation/evaluation and isolated API replay
code are prepared and tested with generated inputs. Actual calibration, DEV
recognition and trained-worker replay for this follow-up remain unverified.
These additions do not resolve the all-signal paired-data gap below.
...
The current metadata-only snapshot, `dataset_budget_type2branch_capture.json`,
counts **1,208,053,730 bytes total** and a conservative largest-source bound of
**465,998,793 bytes**, including temporary fixtures and other derived arrays.
