...
PATH_ARGUMENTS=('features','tap_features','tap_report','tap_diagnostic_dir','selection_dir','output')


def canonical_paths(args):
    return {name:str(getattr(args,name).resolve()) for name in PATH_ARGUMENTS}


def require_bound_paths(args,plan):
    if canonical_paths(args)!=plan.get('canonical_paths'):
        raise ValueError('CLI paths differ from reviewed plan; refuse unpinned artifact access')
...
def main():
    ...
    if args.action=='prepare':
        ...
        if digest(args.features)!=report.get('plan',{}).get('fit_window_archive_sha256'):
            raise ValueError('Encoder feature archive differs from original tap window source')
        ...
        inputs={str(p.resolve().relative_to(ROOT)):digest(p) for p in [args.features,args.tap_features,args.tap_report,
            ...
            ROOT/'research/benchmarks/hmog/tap_train_protocol_v1.json',
