"""TRAIN-fit authorization with explicit quarantine for shared device links."""
import json
import struct
...
def iter_authorized(stream, device_to_user, roles, allowed_users, decoder, *,
                    ...
                    max_document_bytes=MAX_DOCUMENT_BYTES, quarantine_device_ids=()):
...
"""
    ...
    for user in allowed:
        ...
        if (role.get('active') is not True or role.get('identity_role') != 'fit'
                or role.get('partition') != 'train' or role.get('inactive_reason') == 'ambiguous_device_link'):
    ...
    if isinstance(quarantine_device_ids, (str, bytes)):
        raise ValueError('Explicit quarantined-device collection required')
    quarantine = frozenset(quarantine_device_ids)
    for device in quarantine:
        _validate_identifier(device)
    if quarantine & device_to_user.keys():
        raise ValueError('Quarantined devices must be disjoint from unambiguous mapping')
    selected = 0
