...
def main():
    ...
    plan['revision'] = 'v2: sensor width corrected from6 to7 using existing TRAIN schema probe column counts; v1 artifacts preserved'
    with (OUT / 'clock_audit_plan_v2.json').open('x') as f:
    ...
    for session in sessions:
        ...
        for filename in FILES[1:]:
            ...
            rows, invalid = read_rows(reader, session, filename, 7 if kind == 'sensor' else 11)
    ...
    with (OUT / 'clock_audit_results_v2.json').open('x') as f:
