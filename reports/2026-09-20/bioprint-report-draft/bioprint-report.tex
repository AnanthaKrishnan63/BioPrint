...
\]
Here $u$ identifies the account; $E_u$ is its set of enrollment blocks; $j$ indexes blocks; $z_j$ is an encoder vector; $\hat z_j$ is its unit-length form; $\|\cdot\|_2$ is Euclidean length; $c_u$ is the unit-length personal template; and $\mathsf T$ forms the dot product. The last sum uses the first five \emph{challenge} blocks, and $s_p$ is pointer similarity. Acceptance requires $s_p\geq0.948082149$. Five blocks require 641 coordinates, with no padding or cross-session concatenation. The three-second minimum is only a lower bound; source-data median capture time was 16.89 seconds. The encoder uses the same normalization and threshold as the source experiment.

\subsection{Challenge routing and application behavior}
Uncertain desktop attempts use the trained pointer check after pointer enrollment; the existing motor profile remains a fallback. On a phone, two scrambled-keypad runs compare search/cadence against a profile built from six enrollment runs. Current mobile acceptance uses the full personal threshold. Nonces, expiry, account-bound captures and session-cookie checks connect scoring to real access decisions. The application programming interface (API), implemented with FastAPI and SQLite, preserves raw recordings and separate profiles. An invitation-protected encrypted local-network gateway supports cross-device testing. Accepted attempts do not silently update the owner's template.
...
\begin{axis}[width=.93\linewidth,height=43mm,xbar,bar width=5pt,xmin=0,xmax=30,
 ...
 y dir=reverse,enlarge y limits=.17,xlabel={Equal error rate (\%); within-protocol comparisons},
...
\end{axis}
\end{tikzpicture}
\captionof{figure}{Nonlinear typing models and learned pointer representations improve separation. The 100-enrollment CMU result explicitly uses more owner data.}
\end{center}
...
\end{center}
Run \texttt{bash .worktrees/typing-pointer/experiments/typing\_demo/run.sh} from the repository root. The script checks model use, saved-score agreement and session issuance, then opens a local browser report. It requires no live accounts or network listener.

\subsection{Why evaluate general models and genuinely paired fusion?}
...
\par\endgroup
\newpage
\subsection{Choosing the login combination}
All policies face the same 60 genuine and 90 impostor development scenarios. Candidates share compatible learned typing but request different checks when evidence is uncertain. The pointer follow-up replaces statistical movement scoring with the actual trained encoder on mapped source recordings.
\begin{center}\small
...
\begin{center}
\begin{tikzpicture}
\begin{axis}[width=.72\linewidth,height=40mm,xmin=0,xmax=43,ymin=0,ymax=49,
...
\end{axis}
\end{tikzpicture}
\captionof{figure}{The measured operating tradeoffs. Lower left is preferable. These are conditional policies, not points on a common single-score threshold curve; no app-wide EER is inferred.}
\end{center}
...
The central contribution is targeted additional evidence, not indiscriminate feature accumulation. Trained pointer prevents five additional impostor admissions in the controlled comparison while preserving direct login for convincing typing. Residual attacks identify the next precise target: direct-admission verification. This combination of measured improvement, explicit tradeoffs and working integration provides a strong foundation for broader paired-user validation and progressively richer personal models.
...
Local evidence: \texttt{complete-results.csv}, \texttt{evidence-manifest.json}, \texttt{paired-comparison-analysis.json}, and the typing demo's \texttt{results.json}. Dataset-rate tables are generated from saved evidence; rates retain their stated protocols.}
\end{document}
