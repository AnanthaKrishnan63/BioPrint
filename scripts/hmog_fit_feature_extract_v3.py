"""Retained-pair v3 fit-only feature extraction; no model fitting or held-out reads."""
import csv
...
DEST=ROOT/'datasets/hmog/fit_features_v3.npz'
...
def strict_pairs(keys,touch):
    ...
    contacts,_=parse_contacts(touch)
    events=defaultdict(list)
    for row in touch:
        if row[5] in (0,1,5,6):
            events[(row[2],row[1],0 if row[5] in (0,5) else 1)].append(row)
    valid={(c[0][1],c[-1][1],c[0][2],c[0][4]):c for c in contacts}
    accepted=[]
    for p,u,a,k,o in pairs:
        ds,us=events[(a,p,0)],events[(a,u,1)]
        if o!=0 or len(ds)!=1 or len(us)!=1:continue
        d,v=ds[0],us[0]
        if d[4]!=v[4] or (p,u,a,d[4]) not in valid or v[0]<d[0]:continue
        accepted.append((p,u,a,k,d[0],v[0]))
    return sorted(accepted)


def gap_statistics(keys,pp):
    lo=min(p[0] for p in pp);hi=max(p[1] for p in pp);aid=pp[0][2]
    ...
    extra=raw-expected
    allpairs,_=complete_key_pairs(keys)
    complete=set()
    for p,u,a,k,o in allpairs:
        complete.update([(p,a,0,k,o),(u,a,1,k,o)])
    orphan=sum(n for e,n in extra.items() if e not in complete)
    return {'retained_endpoint_ratio':sum(expected.values())/sum(raw.values()),
            'extra_recorded_events':sum(extra.values()),'orphan_or_ambiguous_extra_events':orphan,
            'gap_extra_event_counts':[sum(n for e,n in extra.items() if a[0]<e[0]<b[0]) for a,b in zip(pp,pp[1:])],
            'retained_press_intervals_ms':[b[0]-a[0] for a,b in zip(pp,pp[1:])]}
...
def window_quality(keys,touch,pairs,acc,gyro,start,end,activity):
    ...
    for label,source in [('acc',acc),('gyro',gyro)]:
        ...
        sensors.append(win)
    # v3 explicitly reports omitted key events; unrelated contact events no longer reject the window.
    removed={'cancel_outside_or_multitouch_action','touch_not_single_pointer','touch_nonportrait',
             'invalid_native_contact_span','invalid_intervening_key_event'}
    return [r for r in reasons if r not in removed],pp,sensors
...
def main():
    ...
    plan={'created_at_utc':datetime.now(timezone.utc).isoformat(),'subject_sessions':sessions,
        ...
        'dependency_sha256':{n:digest(ROOT/'scripts'/n) for n in ['hmog_data.py','hmog_clock_audit.py','hmog_behaveformer_features.py','hmog_contact_parser.py']},
    ...
    with (OUT/'fit_feature_v3_plan.json').open('x') as f:json.dump(plan,f,indent=2)
    with (OUT/'fit_feature_v3_source.py').open('x') as f:f.write(Path(__file__).read_text())
    ...
    result={'plan':plan,'subjects':reports,'total_eligible_windows':len(kk),
            'scope':'four fit identities TRAIN only; no population fitting','version':3,
    ...
    with (OUT/'fit_feature_v3_results.json').open('x') as f:json.dump(result,f,indent=2)
