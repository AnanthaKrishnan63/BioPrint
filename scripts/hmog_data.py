...
DEV_V2_SHA256 = '35c1d8077926c62acd6d1354c831b760e3efe7d6d98d5405a3106720bc5383fb'
...
TAP_REFERENCE_SHA256 = '73f4b3b444702e2a055db8feb249e45295f1ab61a940e2631b67822fd9dbd757'
TAP_SELECTION_SHA256 = '68ea937bf24b58f2301cd1d6636d9bfba68f6e9df3c014cf20ef69d588ce9125'
TAP_DEV_EVALUATION_SHA256 = 'aeca15caa5c7567d76603d25c49a138844d940de6b25da7b2a5ba03d7a98ecfc'
...
class HmogReader:
    def __init__(self, *, phase='first_fit_schema'):
        if phase not in ('first_fit_schema', 'all_fit_schema', 'fit_feature_feasibility', 'fit_feature_extraction', 'selection_feature_extraction', 'selection_v2_feature_extraction', 'calibration_v2_feature_extraction', 'dev_v2_feature_extraction', 'tap_reference_feature_extraction', 'tap_selection_feature_extraction', 'tap_dev_evaluation_feature_extraction'):
        ...
        if phase == 'dev_v2_feature_extraction':
            ...
            calibration_protocol = overlay['calibration_protocol']
            if (calibration_protocol['path'] != 'research/benchmarks/hmog/calibration_fallback_protocol_v1.json'
                    or digest(ROOT / calibration_protocol['path']) != calibration_protocol['sha256']
                    or report.get('plan', {}).get('calibration_protocol_sha256') != calibration_protocol['sha256']
                    or report.get('calibration_protocol_sha256') != calibration_protocol['sha256']):
                raise ValueError('HMOG fallback calibration policy checksum/binding mismatch')
        ...
        if phase == 'tap_reference_feature_extraction':
            raw = (BASE / 'tap_reference_feature_extraction_authorization_v1.json').read_bytes()
            if hashlib.sha256(raw).hexdigest() != TAP_REFERENCE_SHA256:
                raise ValueError('HMOG tap authorization checksum mismatch')
            overlay = json.loads(raw)
            for relative, expected in overlay['source_hashes'].items():
                if digest(BASE / relative) != expected:
                    raise ValueError('HMOG tap role provenance mismatch')
            for field, expected_path in [('tap_protocol','research/benchmarks/hmog_tap_protocol_proposal_v1.json'),
                                         ('window_archive','datasets/hmog/fit_features_v3.npz')]:
                info = overlay[field]
                if info['path'] != expected_path or digest(ROOT / expected_path) != info['sha256']:
                    raise ValueError('HMOG tap protocol/window checksum mismatch')
            if (overlay['phase'] != phase or overlay['allowed_purpose'] != phase
                    or set(overlay['allowed_subjects']) != ALL_FIT_SUBJECTS or overlay['allowed_cohort'] != 'fit'
                    or overlay['allowed_sessions'] != list(range(1,17))
                    or set(overlay['allowed_roles']) != {'train_enrollment','train_fit'}
                    or set(overlay['allowed_filenames']) != {'Activity.csv','TouchEvent_im.csv'}):
                raise ValueError('HMOG tap authorization exceeds fixed bounds')
            self.allowed_subjects = ALL_FIT_SUBJECTS
            self.allowed_purpose = phase
            self.allowed_filenames = frozenset(('Activity.csv','TouchEvent_im.csv'))

        if phase == 'tap_selection_feature_extraction':
            raw = (BASE / 'tap_selection_feature_extraction_authorization_v1.json').read_bytes()
            if hashlib.sha256(raw).hexdigest() != TAP_SELECTION_SHA256:
                raise ValueError('HMOG selection tap authorization checksum mismatch')
            overlay = json.loads(raw)
            for relative, expected in overlay['source_hashes'].items():
                if digest(BASE / relative) != expected:
                    raise ValueError('HMOG selection tap role provenance mismatch')
            for field, expected_path in [
                    ('selection_tap_protocol','research/benchmarks/hmog/tap_selection_protocol_v1.json'),
                    ('window_archive','datasets/hmog/selection_features_v1.npz'),
                    ('paired_report','research/benchmarks/hmog/tap_paired_train_v2/paired_train_complete.json')]:
                info = overlay[field]
                if info['path'] != expected_path or digest(ROOT / expected_path) != info['sha256']:
                    raise ValueError('HMOG selection tap protocol/window/paired-report checksum mismatch')
            if (overlay['phase'] != phase or overlay['allowed_purpose'] != phase
                    or set(overlay['allowed_subjects']) != SELECTION_SUBJECTS or overlay['allowed_cohort'] != 'selection'
                    or overlay['allowed_sessions'] != list(range(1,17))
                    or set(overlay['allowed_roles']) != {'train_enrollment','train_selection'}
                    or set(overlay['allowed_filenames']) != {'Activity.csv','TouchEvent_im.csv'}):
                raise ValueError('HMOG selection tap authorization exceeds fixed bounds')
            self.allowed_subjects = SELECTION_SUBJECTS
            self.allowed_cohort = 'selection'
            self.allowed_roles = frozenset(('train_enrollment','train_selection'))
            self.allowed_purpose = phase
        ...
        if phase == 'tap_dev_evaluation_feature_extraction':
            raw = (BASE / 'tap_dev_evaluation_feature_extraction_authorization_v1.json').read_bytes()
            if hashlib.sha256(raw).hexdigest() != TAP_DEV_EVALUATION_SHA256:
                raise ValueError('HMOG dev tap authorization checksum mismatch')
            overlay = json.loads(raw)
            for relative, expected in overlay['source_hashes'].items():
                if digest(BASE / relative) != expected:
                    raise ValueError('HMOG dev tap role provenance mismatch')
            info = overlay['evaluation_protocol']
            if (info['path'] != 'research/benchmarks/hmog/tap_dev_evaluation_protocol_v1.json'
                    or digest(ROOT / info['path']) != info['sha256']):
                raise ValueError('HMOG dev tap evaluation protocol checksum mismatch')
            for info in overlay['bindings'].values():
                path = (ROOT / info['path']).resolve()
                if not path.is_relative_to(ROOT.resolve()) or digest(path) != info['sha256']:
                    raise ValueError('HMOG dev tap source/parameters/window checksum mismatch')
            for filename, expected in overlay['helper_sources'].items():
                if filename not in ('hmog_tap_windows.py','hmog_tap_reference.py','hmog_contact_parser.py') or digest(ROOT/'scripts'/filename) != expected:
                    raise ValueError('HMOG dev tap helper checksum mismatch')
            if (overlay['phase'] != phase or overlay['allowed_purpose'] != phase
                    or set(overlay['allowed_subjects']) != DEV_SUBJECTS or overlay['allowed_cohort'] != 'dev'
                    or overlay['allowed_sessions'] != list(range(1,17))
                    or set(overlay['allowed_roles']) != {'train_support','dev_probe'}
                    or set(overlay['allowed_filenames']) != {'Activity.csv','TouchEvent_im.csv'}):
                raise ValueError('HMOG dev tap authorization exceeds fixed bounds')
            self.allowed_subjects = DEV_SUBJECTS
            self.allowed_cohort = 'dev'
            self.allowed_roles = frozenset(('train_support','dev_probe'))
            self.allowed_purpose = phase
    ...
    def authorize(self, subject, session, filename, *, expected_cohort, expected_role, purpose):
        ...
        if self.allowed_filenames is not None and filename not in self.allowed_filenames:
            raise PermissionError('Member filename is outside named tap scope')
