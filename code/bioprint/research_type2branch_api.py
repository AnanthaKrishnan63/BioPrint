...
from type2branch_release import ROOT, RELEASE, digest, verify_release
from type2branch_wire import validate_features

MANIFEST_SHA256 = None
FAILURE_REPORT_SHA256 = '8839c06256bb97c8cdea33015e153975cb2a729bda322e49a2db4cb860418443'
...
def frozen_failure():
    if FAILURE_REPORT_SHA256 is None:
        return None
    try:
        raw=(ROOT/'research/benchmarks/type2branch_dev_features_v1/report.json').read_bytes()
        if digest(raw)!=FAILURE_REPORT_SHA256:raise ValueError('Failure evidence changed')
        report=json.loads(raw)
        if (report['status']!='infeasible_prescribed_cohort' or report['metrics']!={}
                or not report['failures'] or report['sealed_test_payloads_decoded']):
            raise ValueError('Failure evidence schema changed')
        return report
    except (OSError,ValueError,KeyError,TypeError) as error:
        raise HTTPException(503,'Frozen Type2Branch failure evidence unavailable') from error


def require_available():
    failure=frozen_failure()
    if failure is not None:
        raise HTTPException(409,{'status':failure['status'],'failures':failure['failures'],
                                 'validation_metrics_available':False,'scoring_available':False})
...
async def results():
    failure=frozen_failure()
    if failure is not None:
        return {'status':failure['status'],'validation':failure,
                'validation_metrics_available':False,'scoring_available':False}
